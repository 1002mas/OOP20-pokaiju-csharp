...
public class TestPlayer
{
    ...
    {
        _player?.AddMonster(_monsterA ?? throw new InvalidOperationException());
        _player?.AddMonster(_monsterB ?? throw new InvalidOperationException());
        Assert.AreEqual(2, _player?.GetAllMonsters().Count);
        Assert.False(_player?.GetAllMonsters().Contains(_monsterC ?? throw new InvalidOperationException()));
        Assert.Equals("bibol", _player?.GetAllMonsters()[0].GetName() ?? throw new InvalidOperationException());
        Assert.AreNotEqual("bibol", _player?.GetAllMonsters()[1].GetName());
        Assert.True(_player?.RemoveMonster(_monsterB ?? throw new InvalidOperationException()));
        Assert.False(_player?.RemoveMonster(_monsterC ?? throw new InvalidOperationException()));
    }

    [Test]
    public void ItemListCheck()
    {
        _player?.AddItem(_monsterBall ??throw new InvalidOperationException());
        Assert.True(_player?.GetAllItems().Keys.Any(item => item.Equals(_monsterBall)),
            "Monster Ball doesn't exist in bag");
        Assert.False(_player?.GetAllItems().ContainsKey(_healingPotion ?? throw new InvalidOperationException()));
        _player?.AddItem(_healingPotion ??throw new InvalidOperationException());
        Assert.True(_player?.GetAllItems().ContainsKey(_healingPotion ?? throw new InvalidOperationException()));
        _player?.UseItem(_monsterBall ?? throw new InvalidOperationException());
        Assert.False(_player?.GetAllItems().ContainsKey(_monsterBall ?? throw new InvalidOperationException()));
        _player?.AddItem(_healingPotion ?? throw new InvalidOperationException());
        Assert.AreEqual(2, _player?.GetAllItems()[_healingPotion ?? throw new InvalidOperationException()]);
    }
    ...
    public void UseItemCheck()
    {
        ...
        _player?.useItemOnMonster(_healingPotion ?? throw new InvalidOperationException(), _monsterA ?? throw new InvalidOperationException());
